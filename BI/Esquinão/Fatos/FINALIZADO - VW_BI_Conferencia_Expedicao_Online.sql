/*
    ...
    DATA/HORA MODIFICAÇÂO..: 31/12/2025 11:19 AM
...
IF DBO.OBJECT_ID('VW_BI_Conferencia_Expedicao_Online') IS NOT NULL 
    DROP VIEW VW_BI_Conferencia_Expedicao_Online
GO

CREATE VIEW VW_BI_Conferencia_Expedicao_Online WITH ENCRYPTION
AS

WITH Conferencia_Doca AS (
    SELECT 
        TBretorno_coletor_conferencia_doca.DFdata_hora                                                      AS Data_conferencia,
    ...
    FROM 
        TBretorno_coletor_conferencia_doca WITH(NOLOCK)
        JOIN TBitem_retorno_coletor_conferencia_doca WITH(NOLOCK) ON TBretorno_coletor_conferencia_doca.DFid_retorno_coletor_conferencia_doca = TBitem_retorno_coletor_conferencia_doca.DFid_retorno_coletor_conferencia_doca
        JOIN TBtarefa WITH(NOLOCK) ON TBtarefa.DFid_relacao = TBretorno_coletor_conferencia_doca.DFid_relacao
    ...
    WHERE 
        TBretorno_coletor_conferencia_doca.DFdata_hora >= CAST(GETDATE() AS DATE)
    GROUP BY 
        TBretorno_coletor_conferencia_doca.DFdata_hora,
